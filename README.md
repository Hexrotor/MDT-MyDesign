...
### Mega自动采矿+攻击闪避
调用巨像自动开采核心内最少的物品(铜铅煤钛)同时保留自动修复，可手动选择开采物，扣血量超过10自动返回基地。优先攻击敌人，并反复横跳躲避攻击。对于开采等级低的单位会出问题，可以通过挖矿选择器强制指定开采矿物。本逻辑兼容所有"boostable"的矿机，自行更改unit bind即可。由于技术原因自动修复功能仅对巨像单位开放。<br>
