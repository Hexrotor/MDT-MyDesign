...
Notice: 距离匹配造成匹配效率降低，现默认关闭。要启用，请更改<code>$跳过距离匹配</code>的布尔值。<br>
...
个人感想：原来建造中的建筑类型是build+size；使用随机参数圆的效果不如直接随机坐标，推测是因为圆心角在随机值中能生成的有效份有很大局限性，微小的圆心角变化也会在大半径圆的圆弧上造成很大的差异，这可能使得用这种方式找到空闲区域放置荷载的效率不佳。<br>
